...
- 重新打开剧集时，自动选择同目录最近保存进度的一集，并恢复该集的时间位置。
...
MPV 的 `mpv/series-resume.lua` 随脚本一起安装，因此请下载或克隆完整仓库。
例如第 3 集看到 20 分钟后正常关闭播放器，再打开同目录第 1 集，会跳到第 3 集
上次的位置。手动切换集数不受影响；想临时忽略续播记录，可使用
`mpv --resume-playback=no /path/to/episode.mp4`。文件移动或重命名后，旧进度不一定
能匹配；异常断电也可能来不及保存。不同剧集请分别存放在不同目录。
